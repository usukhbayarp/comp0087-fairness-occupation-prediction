...
def build_lora_model(
    ...
    if full_finetune:
        ...
        print(f"  [full finetune] trainable params: {total:,} || all params: {total:,} || trainable%: 100.00")
    else:
        if use_4bit:
...
def train(args):
    ...
    model, tokenizer = build_lora_model(
        model_name, num_labels, label2id, id2label,
...
def parse_args():
    ...
    p.add_argument("--use_4bit",          action="store_true",
                   help="QLoRA 4-bit quantisation (recommended for 1.4b+, ignored when --full_finetune)")
    p.add_argument("--full_finetune",     action="store_true",
                   help="Full fine-tuning (all parameters); overrides LoRA/QLoRA")
