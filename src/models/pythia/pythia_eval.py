...
def load_finetuned_model(checkpoint_dir: str, label2id: Dict, id2label: Dict,
    ...
    if is_lora:
        ...
        print(f"[model] Loading full fine-tuned checkpoint from {checkpoint_dir} ...")
        ...
        print("[model] Full fine-tuned model loaded. Ready for inference.")
...
def main(args):
    ...
    preds_path = os.path.join(args.output_dir, f"preds_pythia_{size}_{suffix}.jsonl")
    with open(preds_path, "w", encoding="utf-8") as f:
        for rec in predictions:
            f.write(json.dumps(rec, ensure_ascii=False) + "\n")
    print(f"\n[save] Predictions  → {preds_path}")

    metrics_path = os.path.join(args.output_dir, f"metrics_pythia_{size}_{suffix}.json")
    with open(metrics_path, "w") as f:
        json.dump(metrics, f, indent=2)
    print(f"[save] Metrics      → {metrics_path}")

    report_path = os.path.join(args.output_dir, f"clf_report_pythia_{size}_{suffix}.txt")
    y_true = [r["label_true"] for r in predictions]
    y_pred = [r["label_pred"] for r in predictions]
    with open(report_path, "w") as f:
        f.write(f"model: pythia-{size} | regime: {suffix}\n\n")
...
def parse_args():
    ...
    p.add_argument("--suffix",          type=str, default="finetuned",
                   help="Suffix for output filenames, e.g. QLoRA or full")
    return p.parse_args()
