...
drivers of bias---they appear with weights 33--287$\times$ smaller than
...
\textit{Primary contribution} (to our knowledge, limited prior work
on this dataset provides systematic attribution analysis of this kind):
(i)~token-level Integrated Gradients attribution tables stratified by
both profession and gender, with proxy stability and erasure faithfulness
compared across encoder architectures.
\textit{Secondary contribution}:
(ii)~a cross-model comparison of how proxy token distributions shift
under gender masking, quantifying both stability and functional importance.
\textit{Empirical applications of known methods to this setting}:
(iii)~a Pythia scaling study (160M--1.4B) across zero-shot, few-shot,
and fine-tuned regimes;
(iv)~a cross-architecture comparison of encoder and generative model
fairness on Bias-in-Bios;
(v)~a paired masked/unmasked evaluation of gender-token scrubbing on
both task performance and fairness metrics.

\paragraph{Key Findings.}
(1)~Fine-tuned models substantially outperform zero- and few-shot
baselines in both task performance and fairness, consistent with prior
work on task-specific adaptation \citep{de-arteaga-etal-2019-bias,
romanov-etal-2019-whats};
(2)~zero-shot bias \emph{increases} with Pythia scale (EO gap
${\sim}0.09$ at 160M to ${\sim}0.21$ at 1.4B), in line with scaling
risks documented for foundation models \citep{bommasani2021opportunities};
(3)~gender-token masking reduces aggregate bias for encoder models but
produces profession-level backfire effects, consistent with the
documented limits of scrubbing-based debiasing
\citep{romanov-etal-2019-whats, lu-etal-2020-gender};
(4)~gender tokens carry weights 33--287$\times$ smaller than dominant
profession tokens within each model (48--287$\times$ for DistilBERT,
33--158$\times$ for RoBERTa), suggesting bias operates through indirect
contextual signals \citep{de-arteaga-etal-2019-bias}; and
(5)~\emph{novel on this dataset}---proxy stability is model-dependent:
the same profession can be stable under one encoder and unstable under
another.
Full findings are reported in §\ref{sec:fairness} and §6.3.
We do not claim these findings generalise beyond the Bias-in-Bios setting.
...
(\emph{mr, mrs, ms, sir, lady}, etc.), and 35 gendered nouns
...
($K\!=\!4$ exemplar pairs) regimes, with and without gender masking.\footnote{The four exemplars cover professor (female pronoun), physician, journalist, and photographer (all male pronouns). To assess whether this 3:1 male skew introduced exemplar-level bias, we compared few-shot vs.\ zero-shot TPR gaps for these four professions across all three model sizes; no consistent directional effect was observed (full values in Appendix~C), so exemplar gender is unlikely to be a primary confound in the few-shot results.}
...
baselines (RoBERTa (M) 0.880, DistilBERT (M) 0.872); Pythia-160M fine-tuned fails (F1\,$\approx$\,0.02), likely due to label-space collapse under LoRA at insufficient capacity. Zero-shot F1 scales from 0.23 (160M)
...
models. Integrated Gradients and token erasure provide evidence
of functional token importance but do not establish causal
relationships between input features and model predictions;
the scores reflect learned associations, not mechanisms.
...
The few-shot exemplars were not gender-balanced (3 male, 1 female pronouns);
no consistent directional effect on TPR gaps is observable for the four
exemplar professions across model sizes (§4.1, footnote), but a residual
exemplar-level confound cannot be fully excluded.
...
Remaining directions include extending attribution to generative models
to provide mechanistic explanations for observed fairness disparities;
