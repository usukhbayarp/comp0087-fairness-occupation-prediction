...
baselines in both performance and fairness, consistent with the
expected gap between in-context learning and supervised adaptation
\citep{bommasani2021opportunities}.
(2)~Larger models are not inherently fairer: zero-shot bias
\emph{increases} with scale, with the EO gap growing from
${\sim}0.09$ at 160M to ${\sim}0.21$ at 1.4B parameters.
...
(4)~Explicit gender tokens are not the primary drivers of bias:
they carry attribution weights 61--289$\times$ smaller than dominant
profession tokens \citep{de-arteaga-etal-2019-bias}, while indirect
contextual signals carry the latent gender signal.
(5)~\textit{Novel on this dataset}: proxy stability is
model-dependent---the same profession can be stable under one encoder
and unstable under another, though whether this reflects architecture,
pre-training, or scale cannot be determined from this comparison alone.
...
Since Pythia is a causal language model rather than a classifier, occupation prediction is framed as a \textit{candidate scoring} problem. Each candidate label $c_k \in \mathcal{C}$ is scored by its joint log-probability conditioned on the prompt $p$:
\begin{equation} \log P(c_k \mid p) = \sum_{t=1}^{|c_k|} \log P\!\left(c_k^{(t)} \mid p, c_k^{(1)}, \ldots, c_k^{(t-1)}\right) \end{equation}
The predicted label is $\hat{y} = \arg\max_{c_k \in \mathcal{C}} \log P(c_k \mid p)$. A softmax-normalised confidence score over candidates is also recorded as a calibration proxy. All predictions are saved in a shared JSONL format.
...
To assess whether the identified proxy tokens are functionally important
for the prediction rather than merely correlated with it, we perform a
...
Table~\ref{tab:configs} summarises all evaluated configurations. All models are assessed on the shared 3,000-example test subset described in §\ref{sec:evaluation}, ensuring direct comparability across architectures and regimes.
...
\textbf{Masking provides inconsistent improvement.}
For Pythia-1.4B fine-tuned, masking reduces the aggregate EO gap
from 0.086 to 0.080, and encoder reductions are larger
(RoBERTa: 0.082$\to$0.063; DistilBERT: 0.088$\to$0.071).
The effect is non-uniform at the profession level: within Pythia-1.4B
fine-tuned, masking reduces bias for \emph{nurse}, \emph{surgeon},
and \emph{model}, but increases it for \emph{teacher}, \emph{poet},
and \emph{painter} (backfire effect). Pythia-410M fine-tuned shows
aggregate backfire (0.085$\to$0.088), consistent with implicit
proxy signals surviving the removal of explicit gender tokens.
